$(document).ready(function () {
    ...
    $.get('https://countriesnow.space/api/v0.1/countries/positions', function (response) {
        const countries = response.data.map(c => ({
            id: c.name,
            text: c.name,
            iso: countryNameToIso[c.name] || null
        }));
        ...
        // Set selected country if editing
        if (selectedCountry) {
    ...
    countrySelect.on('change', function () {
        ...
        const isoCode = $('#country').select2('data')[0]?.iso || '';
        $('#CountryCode').val(isoCode); // set hidden ISO code
    ...
    function formatCountryOption(option) {
        if (!option.id || !option.iso) return option.text;

        const flagUrl = `https://flagcdn.com/24x18/${option.iso.toLowerCase()}.png`;
        return $(`
            <span>
                <img src="${flagUrl}" style="width: 20px; height: 15px;" class="me-2" />
                ${option.text}
            </span>
        `);
    }
