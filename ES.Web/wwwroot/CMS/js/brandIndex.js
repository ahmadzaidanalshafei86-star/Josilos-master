...
async function toggleStatus(brandId, element) {
    try {
        const ToggleUrl = `${baseUrl}EsAdmin/Brands/ToggleStatus?id=${brandId}`;
        ...
        if (response.status === 200) {
            ...
            Swal.fire({
                icon: 'error',
                title: currentLanguage === 'ar-JO' ? 'ليس لديك صلاحية لتعديل هذه العلامة التجارية!' : 'You don\'t have permission to edit this brand!',
                showConfirmButton: true
            });
        } else {
...
async function deleteBrand(brandId, element) {
    // Confirm deletion using SweetAlert2
    const confirmDeletion = await Swal.fire({
        icon: 'warning',
        title: currentLanguage === 'ar-JO' ? 'هل أنت متأكد أنك تريد حذف هذه العلامة التجارية؟' : 'Are you sure you want to delete this brand?',
    ...
    try {
        const DeleteUrl = `${baseUrl}EsAdmin/Brands/Delete?id=${brandId}`;
        ...
        if (response.ok) {
            ...
            Swal.fire({
                icon: 'success',
                title: currentLanguage === 'ar-JO' ? 'تم حذف العلامة التجارية بنجاح!' : 'Brand deleted successfully!',
                showConfirmButton: true
            });
            ...
            console.error('Failed to delete brand:', responseText);

            // Show permission error using SweetAlert2
            Swal.fire({
                icon: 'error',
                title: currentLanguage === 'ar-JO' ? 'ليس لديك صلاحية لحذف هذه العلامة التجارية!' : 'You don\'t have permission to delete this brand!',
                showConfirmButton: true
            });
        }
    } catch (error) {
        ...
        Swal.fire({
            icon: 'error',
            title: currentLanguage === 'ar-JO' ? 'حدث خطأ أثناء حذف العلامة التجارية.' : 'An error occurred while deleting the brand.',
            showConfirmButton: true
        });
    }
}
