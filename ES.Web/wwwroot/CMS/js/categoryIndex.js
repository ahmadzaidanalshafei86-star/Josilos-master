...
async function deleteCategory(CategoryId, element) {
    console.log('deleteRole called for ID:', CategoryId); // Debug
    ...
    try {
        const DeleteUrl = `${baseUrl}EsAdmin/Categories/Delete?id=${CategoryId}`;
        // Send DELETE request
        ...
        if (response.ok) {
            ...
            console.error('Failed to delete Category:', responseText);
            Swal.fire({
                title: currentLanguage === 'ar-JO' ? "خطأ!" : "Error!",
                text: currentLanguage === 'ar-JO' ? "!! لا يمكن حذف هذا القسم رئيسي او مرتبط بقسم أخرى" : "Can't delete this Parent / Related to another category!",
                icon: "error",
                timer: 3000
            });
...
async function toggleCategoryStatus(categoryId, element) {
    try {
        const ToggoleUrl = `${baseUrl}EsAdmin/Categories/ToggleStatus?id=${categoryId}`;
        // Send the request
